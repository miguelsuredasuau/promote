...
function run(command, args) {
  const result = spawnSync(command, args, {
    ...
    env: { ...process.env, HOME: '/tmp', CI: 'true', TZ: 'UTC', UPDATE_GOLDEN: '0' },
  });
  if (result.error) console.error('[maintenance] command could not finish:', result.error.code ?? 'execution_error');
