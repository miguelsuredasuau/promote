...
export function mountDecisionDemo(root,{onHistory}={}){
 ...
 const buttons=el('div',undefined,'decision-actions');const plan=el('button',d.resolution?'Decisión de planificación registrada':'Encargar planificación');plan.disabled=busy||!!d.resolution;plan.onclick=()=>act(d,false);buttons.append(plan);
 const run=el('button',d.execution.maxAcu?`Ejecutar reparación · máximo ${d.execution.maxAcu} ACU`:'Ejecución pendiente de alcance');run.disabled=busy||!d.execution.canExecute;run.onclick=()=>act(d,true);buttons.append(run);sheet.append(buttons);
