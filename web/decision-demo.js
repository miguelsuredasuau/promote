...
export function mountDecisionDemo(root,{onHistory}={}){
 ...
 async function act(d,execute){busy=true;notice=execute?'Enviando la tarea autorizada a Devin…':'Guardando el encargo de planificación…';render();try{
 ...
 }catch(e){notice=e.message;}finally{busy=false;last='';await refresh();}}
 ...
 const run=el('button',d.execution.releaseId?'Primera reparación publicada':d.execution.candidateSha?'Candidato en verificación':d.execution.remoteId?'Reparación enviada a Devin':d.execution.maxAcu?`Ejecutar reparación · máximo ${d.execution.maxAcu} ACU`:'Ejecución pendiente de alcance');run.disabled=busy||!d.execution.canExecute;run.onclick=()=>act(d,true);buttons.append(run);sheet.append(buttons);
 if(d.execution.releaseId){const chat=el('a','Abrir Xarts Chat con la versión activa →');chat.href='http://127.0.0.1:4320/';chat.target='_blank';chat.rel='noopener';sheet.append(chat,el('p',`Versión publicada: ${d.execution.releaseId}`));}
