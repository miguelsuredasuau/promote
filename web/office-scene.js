...
export function createOfficeScene(host, { onSelect = () => {}, detailElement, nativeSurfaces = {} } = {}) {
  ...
  function update(next) {
    ...
    paint(board, '#fffcf0', (c, w) => {
      text(c, model.maintenancePipeline?`WORK QUEUE · ${model.maintenancePipeline.total} APPROVED`:'WORK QUEUE', 32, 22, 30);text(c,model.maintenancePipeline?`${model.maintenancePipeline.checking} checking · ${model.maintenancePipeline.attention} need attention · Scroll to browse`:'Scroll to browse · click a note to inspect',530,30,21,'#697a6c'); const cols = model.kanban?.columns || []; const cw = (w - 50) / Math.max(cols.length, 1);
    ...
    const q = model.qa || {};
    paint(qaConsole, '#eef5f4', c => { text(c, q.executionMode==='maintenance'?(q.historical?'Last recorded quality result':'Independent quality review'):'Quality', 35, 30, 44, '#214e62'); text(c, q.headline || (q.candidateId ? (q.status==='failed'?'Stopped · review the failed gate':q.status==='completed'?'Checks complete':'Candidate in verification') : 'Waiting for a candidate'), 35, 98, 30, '#45686d'); if(q.executionMode==='maintenance'){text(c,q.task,35,150,26,'#45686d',1000);text(c,`Attempt ${q.attempt} · Candidate ${(q.candidateId??'').slice(0,12)}`,35,220,24,'#45686d',1000);text(c,q.progress?.next??'Await independent evidence.',35,292,26,'#214e62',1000);text(c,'Verification does not publish a release.',35,390,22,'#45686d',1000);return;} (q.stages || []).forEach((stage,i) => { const copy = qaStageCopy(stage, model.mode); text(c, copy.title, 35, 168+i*76, 26, '#214e62'); text(c, copy.headline, 360, 168+i*76, 26, stage.outcome === 'fail' ? '#a34e42' : '#45686d', 700); }); });
    gateLabels.forEach((s, i) => { const stage = q.stages?.[i]; const copy = q.executionMode==='maintenance'?{title:['Candidate','TypeScript','Tests','Release'][i],headline:i===3?'Separate decision':[['identity','preparing'],['typecheck'],['regressions']][i]?.includes(q.progress?.stage)?'Current observed step':'No gate verdict recorded'}:qaStageCopy(stage || { id: ['provenance', 'meaning', 'regression', 'release'][i] }, model.mode); stageNodes[i].textContent = copy.title; stageNodes[i].dataset.outcome = stage?.outcome || 'not_run'; stageNodes[i].title = copy.headline; paint(s, ['#2858b7','#187d80','#be5e3d','#b88a26'][i], c => { text(c, copy.title, 18, 20, 44, '#f4efdf', 480); text(c, copy.headline, 18, 94, 31, '#b5c6a0', 480); }); const outcome = stage?.outcome; gates[i].material.color.setHex(outcome === 'pass' ? P.green : ['fail', 'error'].includes(outcome) ? P.red : outcome === 'pending' ? P.amber : P.sage);gates[i].material.emissive.copy(gates[i].material.color);gates[i].material.emissiveIntensity=outcome==='not_run'||!outcome?0:.22; });
  ...
  function click(e) {
    ...
    if(selected && surfaces[selected]){
      ...
      if(hit){
        ...
        if(selected==='qa'){
          if(model.qa?.executionMode==='maintenance'){nativeDetail={surface:'qa',title:model.qa.task,headline:model.qa.headline,body:[model.qa.progress?.next,model.qa.candidateId?'Candidate '+model.qa.candidateId:null,'Verification does not publish a release.'].filter(Boolean).join(' ')};signature='';update(model);return;}
        ...
        if(selected==='backlog'){
          ...
          if(card&&j>=0){nativeDetail={surface:'backlog',title:card.title,headline:card.detail??card.status,body:[card.next,card.attempt?'Attempt '+card.attempt:null,card.remoteId?'Session '+card.remoteId:null,card.candidateSha?'Candidate '+card.candidateSha:null,card.id].filter(Boolean).join(' · ')};signature='';update(model);return;}
        }
      }
      return;
    }
