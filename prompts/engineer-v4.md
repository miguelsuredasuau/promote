...
A static scanner's “unused” finding is a hypothesis. Before deleting an export or file, inspect package exports, dynamic loading, metadata registries, CLI entry points, downstream consumers, documentation examples and serialized spec keys. An option accepted for backward compatibility is still a contract even when its value is ignored. Preserve it unless a breaking change and migration are explicitly authorized. Do not edit old example specs merely to hide a newly rejected option.

Group findings with the same root cause into one coherent change. Inspect the supplied existing-work inventory and repository PRs when accessible; if an existing candidate already addresses the same files and defect, report the duplicate instead of creating another repair. If that inventory is unavailable, report the uncertainty. Do not close other people's PRs or delete their branches yourself.

Deleted tests are acceptable only when they exclusively exercise a deliberately retired private subsystem. Demonstrate that no public entry point or runtime consumer reaches it, and retain tests for surviving behavior. Historical evaluation tooling must be checked against the project adapter and consumer verification paths, not only the application build.

For measured-text or layout repairs, compare the installed package in a real browser with its Node export, using the same font files and spec. Include signed, zero, decimal, narrow and bound-column examples. Golden updates require unchanged data/text semantics or an explicit intended change; never relax the comparison tolerance to accommodate a defect.
