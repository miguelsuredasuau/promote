...
it.each(['skipped','neutral',null])('does not merge when checks did not execute successfully: %s',async conclusion=>{
 const calls:{url:string;init?:RequestInit}[]=[];
 const fetchImpl=github({'GET /repos/o/r/pulls/7':pull(),[`GET /repos/o/r/commits/${sha('a')}/check-runs?per_page=100&page=1`]:{check_runs:[{name:'ci',status:'completed',conclusion,html_url:null}]},[`GET /repos/o/r/commits/${sha('a')}/status?per_page=100&page=1`]:{statuses:[]}},calls);
...
it('refuses a clean PR with no checks',async()=>{
 const fetchImpl=github({'GET /repos/o/r/pulls/7':pull(),[`GET /repos/o/r/commits/${sha('a')}/check-runs?per_page=100&page=1`]:{check_runs:[]},[`GET /repos/o/r/commits/${sha('a')}/status?per_page=100&page=1`]:{statuses:[]}});
 expect(await mergePullRequest(config,store(),{repo:'o/r',number:7,headSha:sha('a')},{fetchImpl})).toMatchObject({state:'refused'});
});
