...
The studio uses procedural adult staff, neutral daylight, tall transparent glass, contemporary furniture, a side slide and exercise-ball seating. No third-party model assets are bundled. Xarts branding remains a local texture; transparency is configured before shader compilation. Close-up DOM dimensions match final projected screen pixels so typography does not shrink with small world-space objects. Mobile surfaces scroll internally and preserve access to the return control.

## Studio materials and inspection review

The source-authored furniture/material modules add curved upholstery, mesh seating, botanical blades, terrazzo, oak and fabric relief. Lighting uses a locally bundled CC0 HDR environment; see [asset provenance](assets/README.md). The Xarts logo does not cast a rectangular shadow. Staff remain stylized articulated geometry, not photorealistic scanned people.

The ticker is a sixth inspectable surface. Its UV marquee runs in the overview, pauses under reduced motion and hidden tabs, and expands its physical frame for a six-metric close-up. Unknown usage/errors/costs are not synthesized.

Inspection registers accessible HTML to the exact mesh face using the same orthographic camera. Whole foreground objects are hidden during inspection, avoiding both DOM-over-geometry seams and sliced geometry from near-plane clipping. Controls fade in after the approach starts; reduced motion applies the final state immediately. Finance opens its door beyond 90 degrees. QA framing includes all four physical gate signs. Technical gate definitions remain available inside an expandable evidence section.

Six desktop screenshots and mobile checks were recorded locally in `.local/office-review.html`. Local screenshots are not published because they can include the checkout's private branding or records. The review checks include frame continuity, surface boundaries, focus, non-modal status, internal scrolling, return navigation and browser errors. These checks are not proof that the scene matches the photorealistic concept.
