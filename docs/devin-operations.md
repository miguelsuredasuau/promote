...
Deadlines, cancellation and exhausted session ceilings request termination; only observed provider exit confirms it. Returned engineering candidates and completed exploration reports trigger termination and collection. Candidate SHA remains recorded if a later observation omits it. Stopped sessions remain eligible for read-only observations because billing can arrive later.
