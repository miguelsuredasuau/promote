...
export async function progress(){
 ...
 const files=storage().indexFiles();return {batch:{jobs:images.jobs,missing:missingItems(assets,images.jobs).map(a=>({id:a.id,name:a.name})),active:images.jobs.filter(j=>['planned','submitting','queued','generating'].includes(j.status)).length},draft:storage().get('draft',{}),storage:{engine:'SQLite',fileCount:files.length},files,decisionHistory:storage().history('decisions'),room,requirements,roomApproved,revision:decisions.revision,review,brief:pilot?.brief??null,assets,events:decisions.events,requests:pilot?.assets.filter(a=>a.requestId).length??0,ready,approved:assets.filter(a=>a.approved).length,actualCost:null,stages:[
