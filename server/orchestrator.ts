...
const ClassificationRecord = z.object({
 ...
 observations:z.object({summary:z.object({release:z.union([z.string().min(1).max(2000),historicalRelease]).nullish()}).passthrough().nullish(),
...
export function classifyRecord(input:unknown):Proposal[] {
 ...
 if(record.schema==='promote/exploration@1'){
  ...
  for(const shim of (typeof release==='object'&&release?release.shims??[]:[]))add('bug',{kind:'packaging_workaround',code:shim.id},`Remove package workaround: ${shim.id}`,90,'Reproduce installation outside the monorepo and run the standalone consumer check.');
 }
 return proposals;
}
